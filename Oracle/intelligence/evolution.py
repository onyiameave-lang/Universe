...
class EvolutionLab:
    def __init__(self, chronicle=None, atlas=None, storage_dir: str = "memory",
        ...
        self._champions: Dict[str, Dict[str, Any]] = {}   # SYMBOL::regime -> champion genome dict
        self._load()
    ...
    def _fitness(self, genome: StrategyGenome, series) -> Dict[str, Any]:
        ...
        # Scientific fitness: reward repeatable risk-adjusted return, penalize drawdown.
    ...
    def evolve(self, series, generations: int = 5, planned_candidates: List[StrategyGenome] = None) -> Dict[str, Any]:
        ...
        # split real history: in-sample (fitness) + out-of-sample (certification)
        ...
        # seed population (+ optionally bias mutation toward Atlas-recommended ideas)
        population = [random_strategy(self.rng) for _ in range(self.population_size)]
        
        # Insert planned candidates from Strategy Planner
        if planned_candidates:
            for i, cand in enumerate(planned_candidates[:self.population_size // 2]):
                population[i+1] = cand

        # carry the current champion in as an elite, if any
        ...
        for gen in range(generations):
            ...
            next_pop = list(survivors[:2])  # elites
        ...
        # ---- CERTIFY the best on OUT-OF-SAMPLE (overfitting guard) ----
        ...
        promoted = False
        # require: positive out-of-sample return AND beating incumbent AND enough trades
...
def _slice_series(series, start: int, end: int):
    """Return a lightweight series view over a bar range (no lookahead across the split)."""
    class _S:
        pass
    s = _S()
